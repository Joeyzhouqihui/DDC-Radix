...
public:
  DSMKeeper(ThreadConnection **thCon, DirectoryConnection **dirCon, RemoteConnection *remoteCon,
    ...
    Debug::notifyInfo("Finish initLocalMeta");

    if (!connectMemcached()) {
      return;
    }

    Debug::notifyInfo("Finish connectMemcached");

    serverEnter();

    Debug::notifyInfo("Finish serverEnter");

    serverConnect();

    Debug::notifyInfo("Finish serverConnect");

    connectMySelf();

    Debug::notifyInfo("Finish connectMySelf");

    initRouteRule();

    Debug::notifyInfo("Finish initRouteRule");
  }
