...
DSM::DSM(const DSMConfig &conf)
  ...
  lockAddr = (uint64_t)malloc(define::kLockChipMemSize);

  Debug::notifyInfo("shared memory size: %dGB, 0x%lx", conf.dsmSize, baseAddr);
  Debug::notifyInfo("lock memory size: %d", define::kLockChipMemSize);
  ...
  memset((char*)lockAddr, 0, define::kLockChipMemSize);

  initRDMAConnection();
  
  Debug::notifyInfo("finish initRDMAConnection");
  ...
  Debug::notifyInfo("finish Directory");
...
void DSM::initRDMAConnection() {
  ...
  Debug::notifyInfo("Finish ThreadConnection");

  for (int i = 0; i < NR_DIRECTORY; ++i) {
    dirCon[i] =
        new DirectoryConnection(i, (void *)baseAddr, conf.dsmSize * define::GB,
                                (void *)lockAddr, define::kLockChipMemSize,
                                conf.machineNR, remoteInfo);
  }

  Debug::notifyInfo("Finish DirectoryConnection");

  keeper = new DSMKeeper(thCon, dirCon, remoteInfo, conf.machineNR);

  Debug::notifyInfo("Finish DSMKeeper");
