#ifndef RADIX_KEY_H
#define RADIX_KEY_H

#include <stdint.h>
#include <cstring>
#include <assert.h>

using KeyLen = uint32_t;

class VarKey {
public:
  VarKey() {}
  ~VarKey();
  VarKey(const VarKey &key) = delete;
  VarKey(VarKey &&key);
  void set(char bytes[], const std::size_t length);
  void operator=(const char key[]);
  bool operator==(const VarKey &k) const;
  uint8_t &operator[](std::size_t i);
  const uint8_t &operator[](std::size_t i) const;
  KeyLen getKeyLen() const;
  void setKeyLen(KeyLen len);

private:
  static constexpr uint32_t stackLen = 128;
  uint32_t len = 0;
  uint8_t *data;
  uint8_t stackKey[stackLen];

};

inline VarKey::~VarKey() {
  if (len > stackLen) {
    delete[] data;
    data = nullptr;
  }
}

inline VarKey::VarKey(VarKey &&key) {
  len = key.len;
  if (len > stackLen) {
    data = key.data;
    key.data = nullptr;
  } else {
    memcpy(stackKey, key.stackKey, key.len);
    data = stackKey;
  }
}

inline void VarKey::set(char bytes[], const std::size_t length) {
  if (len > stackLen) {
    delete[] data;
  }
  if (length <= stackLen) {
    memcpy(stackKey, bytes, length);
    data = stackKey;
  } else {
    data = new uint8_t[length];
    memcpy(data, bytes, length);
  }
  len = length;
}

inline void VarKey::operator=(const char key[]) {
  if (len > stackLen) {
    delete[] data;
  }
  len = strlen(key);
  if (len <= stackLen) {
    memcpy(stackKey, key, len);
    data = stackKey;
  } else {
    data = new uint8_t[len];
    memcpy(data, key, len);
  }
}

inline bool VarKey::operator==(const VarKey &k) const {
  if (k.getKeyLen() != getKeyLen()) {
    return false;
  }
  return std::memcmp(&k[0], data, getKeyLen()) == 0;
}

inline uint8_t& VarKey::operator[](std::size_t i) {
  assert(i < len);
  return data[i];
}

inline const uint8_t& VarKey::operator[](std::size_t i) const {
  assert(i < len);
  return data[i];
}

inline KeyLen VarKey::getKeyLen() const { return len; }

inline void VarKey::setKeyLen(KeyLen newLen) {
  if (len == newLen) return;
  if (len > stackLen) {
    delete[] data;
  }
  len = newLen;
  if (len > stackLen) {
    data = new uint8_t[len];
  } else {
    data = stackKey;
  }
}

#endif